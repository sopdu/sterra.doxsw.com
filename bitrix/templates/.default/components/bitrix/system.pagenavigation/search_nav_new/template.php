...
?>
<br>
<div class="kilo tabs-page pagination">
	...
	<div class="navigation">
		...
		<?
			$query = explode("&amp;", $arResult["NavQueryString"]);
			$k = array_search('showAll=Y', $query);
			if ($k!==false) {
				unset($query[$k]);
			}
			$arResult["NavQueryString"] = implode("&amp;", $query);
		?>
		<a href="<?=$arResult["sUrlPath"]?>?<?=$arResult["NavQueryString"]?>" class="right">По страницам</a>
		<?else:?>
		<a href="<?=$arResult["sUrlPath"]?>?<?=$strNavQueryString?>showAll=Y" class="right"><?=GetMessage("nav_all")?></a>
		<?endif;?>
