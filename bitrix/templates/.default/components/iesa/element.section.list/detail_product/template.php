...
<div class="twelve right">
	...
	<?foreach($arResult['ITEMS'] as $arItems):?>
		...
		<div class="box">
			...
			<?if(!empty($arItems['PROPERTIES']['DOCUMINTATION']['DETAIL_PAGE_URL'])):?>
				<div class="twelve">			
					<a href="<?=$arItems['PROPERTIES']['DOCUMINTATION']['DETAIL_PAGE_URL']?>#id<?=$arItems['PROPERTIES']['DOCUMINTATION']['ID']?>" class="doc_button">
						<?=GetMessage('PR_DOCK')?><div class="icons icon-arrow-right-news"></div>
					</a>	
				</div>
			<?else:?>
				<div class="twelve">			
					<a href="/podderzhka/documentation/#<?=$arResult['SECTION'][0]["ID"]?>" class="doc_button">
						<?=GetMessage('PR_DOCK')?><div class="icons icon-arrow-right-news"></div>
					</a>	
				</div>	
			<?endif?>			
