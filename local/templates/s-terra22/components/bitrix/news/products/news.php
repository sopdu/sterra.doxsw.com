...
<div class="catalog-product-page">
    ...
    <div class="catalog-block">
        <div class="container">
            <div id="content" class="catalog-block__content js-catalog" data-full-length="<?=$count?>" data-page-size="10" data-page-size-mobile="5" data-action="/local/ajax/catalog-product.php" data-method="GET">
                <div class="catalog-block__top">
                    <div class="catalog-block__title">Каталог</div>
                    <ul class="catalog-block__filter">
                        <li class="catalog-block__filter__item"><a class="catalog-block__filter__button active" data-type="" href="/products/catalog/#content">Продукты</a></li>
                        <li class="catalog-block__filter__item"><a class="catalog-block__filter__button" data-type="1" href="/catalog/complex_solutions/#content">Комплексные решения</a></li>
                        <li class="catalog-block__filter__item"><a class="catalog-block__filter__button" data-type="2" href="/resheniya/industry_solutions/#content">Отраслевые решения</a></li>
                    </ul>
                </div>
                <div class="catalog-block__grid">
                    <div class="catalog-block__filter-col">
                        <div class="catalog-block__filter-col__top">
                            ...
                            <div class="catalog-block__filter-col__search">
                                <div class="search-field js-input-search-field">
                                    ...
                                    <input data-js-ref="catalog-filter-search" class="js-input-search form-control" value="" placeholder="Поиск" name="q" data-field autocomplete="off">
                        ...
                        $filter = Array(
                            ...
                            array(
                                "LOGIC" => "OR",
                                array("!UF_CERT_CLASS" => false),
                                array("!UF_CERT_FBI" => false),
                                array("!UF_CERT_OTHER" => false),
                            ),
                        );
                        $sections = CIBlockSection::GetList(
                            Array('UF_CERT_FBI' => 'ASC', 'UF_CERT_CLASS' => 'ASC', 'UF_CERT_OTHER' => 'ASC'),
                            $filter,
                            false,
                            Array('UF_CERT_CLASS', 'UF_CERT_FBI', 'UF_CERT_OTHER')
                        );
                        while ($section = $sections->Fetch()){
                            /*echo "<pre>";
                            print_r($section);
                            echo "</pre>";*/
                            $filterValuesClass = array_merge($filterValuesClass, array_flip($section['UF_CERT_CLASS']));
                            $filterValuesFBI = array_merge($filterValuesFBI, array_flip($section['UF_CERT_FBI']));


                            foreach ($section['UF_CERT_OTHER'] as $cert){
                                $filterValuesOther[$cert] = 1;
                            }
                        }
                        ?>

                        <div class="catalog-block__filter-col__checkboxes">
                            <?if(count($filterValuesClass) || count($filterValuesFBI) || count($filterValuesOther)):
                                ksort($filterValuesClass);
                                ksort($filterValuesFBI);
                                ksort($filterValuesOther);

                                $iblock = 71;
                                $sortElemDB = CIBlockElement::GetList(
                                    Array('SORT' => 'ASC'),
                                ...
                                $sortElem = $sortElemDB->Fetch();

                                $res = CIBlockElement::GetProperty($iblock, $sortElem['ID']);
                                $certificationOrderArr = [];
                                while ($ob = $res->GetNext()) {
                                    if ($certificationOrderArr[$ob['CODE']]) $certificationOrderArr[$ob['CODE']]['VALUE'][] = $ob['VALUE'];

                                    else{
                                        $certificationOrderArr[$ob['CODE']] = $ob;
                                        $certificationOrderArr[$ob['CODE']]['VALUE'] = [];
                                        $certificationOrderArr[$ob['CODE']]['VALUE'][] = $ob['VALUE'];
                                    }
                                }
                                $certificationOrder = $certificationOrderArr['CERT']['VALUE'];
                                
                                ?>
                            <div class="catalog-block__filter-col__checkboxes__item">
                                <div class="catalog-block__filter-col__checkboxes__item__title">Сертификация</div>
                                <?
                                foreach ($certificationOrder as $cert):
                                if (isset($filterValuesClass[$cert])){
                                    $isClass = true;
                                    unset($filterValuesClass[$cert]);
                                } else $isClass = false;
                                if (isset($filterValuesFBI[$cert])){
                                    $isFBI = true;
                                    unset($filterValuesFBI[$cert]);
                                } else $isFBI = false;
                                if (isset($filterValuesOther[$cert])){
                                    $isOther = true;
                                    unset($filterValuesOther[$cert]);
                                } else $isOther = false;

                                if ($isClass || $isFBI || $isOther):
                                ?>
                                    <div class="form-field">
                                        <label class="checkbox">
                                            <input type="checkbox" name="cert-<?=$cert?>" data-js-ref="catalog-checkbox-filter">
                                            <div class="checkbox-box"></div>
                                            <div class="checkbox-label"> <?=$cert?></div>
                                        </label>
                                    </div>
                                <?endif; endforeach;?>
                                <? foreach ($filterValuesFBI as $val => $number):?>
                                    <div class="form-field">
                                        <label class="checkbox">
                                            <input type="checkbox" name="cert-<?=$val?>" data-js-ref="catalog-checkbox-filter">
                                            <div class="checkbox-box"></div>
                                            <div class="checkbox-label"> <?=$val?></div>
                                        </label>
                                    </div>
                                <?endforeach;?>

                                <? foreach ($filterValuesClass as $val => $number):?>
                                    <div class="form-field">
                                        <label class="checkbox">
                                            <input type="checkbox" name="cert-<?=$val?>" data-js-ref="catalog-checkbox-filter">
                                            <div class="checkbox-box"></div>
                                ...
                                <? foreach ($filterValuesOther as $val => $number):?>
                                    <div class="form-field">
                                        <label class="checkbox">
                                            <input type="checkbox" name="cert-<?=$val?>" data-js-ref="catalog-checkbox-filter">
                                            <div class="checkbox-box"></div>
    ...
    <div class="catalog-description-block">
        <div class="catalog-description-block__wrap">
            ...
            <div class="catalog-description-block__description">
                <?
                $APPLICATION->IncludeComponent(
                	"bitrix:main.include",
                	"",
                	Array(
                		...
                		"EDIT_TEMPLATE" => "",
                		"PATH" => "/local/include/catalog/advantages.php"
                	)
                );

                $advantages = [];
                $advDB = CIBlockElement::GetList(
                    Array('SORT' => 'ASC'),
                    Array('IBLOCK_ID' => 68),
                    false,
                    false,
                    Array('PREVIEW_TEXT', 'PROPERTY_ICON')
                );
                while ($adv = $advDB->Fetch()) $advantages[] = $adv;
                ...
                <div class="catalog-description-block__items_col">
                    ...
                    <div class="catalog-description-block__item">
                        <div class="catalog-description-block__item-icon">
                            <?if ($advantage['PROPERTY_ICON_VALUE']):
                                $url = CFile::GetPath($advantage['PROPERTY_ICON_VALUE'])
                                ?>
                            <img src="<?=$url?>" alt="" class="advantage-icon">
                            <?endif?>
                        </div>
                        <div class="catalog-description-block__item-text"><?=$advantage['PREVIEW_TEXT']?></div>
                    </div>
                    <?if ($key == $countCol):?>
                    </div>
                    <div class="catalog-description-block__items_col">
            ...
            <div class="catalog-description-block__lists">
                ...
                $APPLICATION->IncludeComponent(
                	...
                	Array(
                		...
                		"AREA_FILE_SUFFIX" => "inc",
                		"EDIT_TEMPLATE" => "",
                		"PATH" => "/local/include/catalog/list.php"
                	)
                );
                ?>
            </div>
