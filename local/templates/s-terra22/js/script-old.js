...
(function() {
  ...
  function createEmitter() {
    ...
    function removeEventCb(eventName, cb) {
      ...
      if (!cb) cbs[eventName] = [];
      else cbs[eventName] = cbs[eventName].filter(function(_cb) {
        return cb === cb;
      });
    }
    ...
    function toggleItem(item) {
      ...
      if (activeItem) {
        ...
        if (yActive > yOpened) {
          ...
          scrollTo(activeItem.trigger.getBoundingClientRect().top + window.scrollY - 100, 400);
        }
      }
