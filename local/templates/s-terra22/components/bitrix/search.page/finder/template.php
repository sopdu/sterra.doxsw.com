...
?>
<div class="search-page page-main">
    ...
    echo "</pre>";*/
    ?>
    <div class="container form-js" data-full-length="<?=$arResult['NAV_RESULT']->NavRecordCount?>" data-page-size="<?=$arResult['NAV_RESULT']->NavPageSize?>" data-action="/local/ajax/search.php" data-method="GET">
          <div class="text-container js-search-page">
	...
	<form action="" method="get">
		...
		<div class="search-field js-input-search-field">
			...
			<?else:?>
				...
				<input class="js-input-search form-control" type="text" placeholder="Поиск по сайту" data-field autocomplete="off" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>" />
			<?endif;?>
            ...
            <script>
                document.querySelector('.js-input-search-clear').addEventListener('click', function () {
                    document.querySelector('.search-page__items').innerHTML = '';
                    document.querySelector('.search-page__info').innerHTML = 'Найдено: 0';
                })
            </script>
			<input style="display:none;" class="blue right" type="submit" value="<?echo GetMessage("CT_BSP_GO")?>" />
		...
		<div class="search-advanced">
			<div class="search-advanced-result">
				<?if(is_object($arResult["NAV_RESULT"])):?>
					<div class="search-page__info"><?echo GetMessage("CT_BSP_FOUND")?>: <?echo $arResult["NAV_RESULT"]->SelectedRowsCount()?></div>
				<?endif;?>
				<?
				$arWhere = array();
...
<?if(isset($arResult["REQUEST"]["ORIGINAL_QUERY"])):
	...
	<div class="search-page__items js-body">
	...
	<?elseif($arResult["ERROR_CODE"]!=0):?>
        ...
        <noindex>
            <div class="search-advanced">
                <div class="search-advanced-result">
                    <div class="search-page__info">Найдено: 0</div>
                </div>
            </div>
        </noindex>
        <?=GetMessage("CT_BSP_EMPTY")?>
		
	<?elseif(count($arResult["SEARCH"])>0):?>
		<?if($arParams["DISPLAY_TOP_PAGER"] != "N") echo $arResult["NAV_STRING"]?>
		<?foreach($arResult["SEARCH"] as $arItem):?>
			<div class="search-page__item">
				<div class="search-page__item__title"><a style="text-decoration: none;" href="<?echo $arItem["URL"]?>"><?echo str_replace($_GET['q'], '<b>'.$_GET['q'].'</b>', $arItem["TITLE_FORMATED"])?></a></div>
				<div class="search-page__item__description"><?echo str_replace($_GET['q'], '<b>'.$_GET['q'].'</b>', $arItem["BODY_FORMATED"])?></div>
			</div>	
				<?if(
		...
		<?if($arParams["SHOW_ORDER_BY"] != "N"):?>
			...
			<?if($arResult["REQUEST"]["HOW"]=="d"):?>
				<a href="<?=$arResult["URL"]?>&amp;how=r"><?=GetMessage("CT_BSP_ORDER_BY_RANK")?></a>&nbsp;<?=GetMessage("CT_BSP_ORDER_BY_DATE")?>
			<?else:?>
				<?=GetMessage("CT_BSP_ORDER_BY_RANK")?>&nbsp;<a href="<?=$arResult["URL"]?>&amp;how=d"><?=GetMessage("CT_BSP_ORDER_BY_DATE")?></a>
			<?endif;?>
			</div>
		<?endif;?>
	<?else:?>
